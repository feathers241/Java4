...
.select-box select:focus {
  ...
  .editor{display:grid;grid-template-columns:300px 1fr;gap:16px}
    ...
    .actions-row{margin-top:12px;background:#f3f3f3;padding:10px;border-radius:4px;display:flex;gap:12px;justify-content:flex-start}
  ...
  <header class="admin-header">
    <div class="wrap">
      ...
      <nav class="admin-nav">
        ...
        <a href="#?adminid=${adminid}">History</a>
    ...
    <form method = "post">
      <div id="edition" class="tab-panel" style = "display:${EditorDisplay}">
        <div class="editor">
          ...
          <div>
            <div class="field"><label for="ytid">History Id?</label><input id="ytid" disabled type="text" value = "${Youtubeid}" name = "Youtubeid"></div>
            <div class="field"><label for="title">User</label><input id="title" type="text" value = "${VideoTitle}" name = "VideoTitle"></div>
            <div class="field"><label for="views">Video</label><input id="views" type="text" value="${ViewCount}" name = "ViewCount"></div>
            <div class="field"><label for="desc">View Date</label><textarea id="desc" name = "Description">${Description}</textarea></div>

            <div class="actions-row">
              <button class="btn create" id="btnCreate" formaction = "${pageContext.request.contextPath}/HistoryManager/Create">Create</button>
              <button class="btn update" id="btnUpdate" formaction = "${pageContext.request.contextPath}/HistoryManager/Update">Update</button>
              <button class="btn delete" id="btnDelete" formaction = "${pageContext.request.contextPath}/HistoryManager/Delete">Delete</button>
              <button class="btn reset" id="btnReset" formaction = "${pageContext.request.contextPath}/HistoryManager/Reset">Reset</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="list" class="tab-panel"  style = "display:${ListDisplay}" >
          ...
          <tbody>
          	<c:forEach items = "${VideoList}" var = "v">
          		 <tr>
		            ...
		            <td>${v.video.id}</td>
		            <td>${v.viewDate}</td>
		            <td><a href = "${pageContext.request.contextPath}/HistoryManager?videoid=${v.id}">Edit</a></td>
           		 </tr>
          	</c:forEach>
          </tbody>
        </table>
        ...
        <div class="pager">
          <a href = "${pageContext.request.contextPath}/HistoryManager?page=1&display=block"><button class="pgbtn" id="first">|&lt;</button></a>
          <a href = "${pageContext.request.contextPath}/HistoryManager?page=${page > 1 ? page-1 : 1}&display=block"><button class="pgbtn" id="prev">&lt;&lt;</button></a>
          
          <div id="pageInfo" style="padding:6px 10px;color:#666"></div>
          
          <a href = "${pageContext.request.contextPath}/HistoryManager?page=${page < 1 ? 1 : page+1}&display=block"><button class="pgbtn" id="next">&gt;&gt;</button></a>
          <a href = "${pageContext.request.contextPath}/HistoryManager?page=${last}&display=block"><button class="pgbtn" id="last">&gt;|</button></a>
        </div>
      </div>
    </div>
  </main>
