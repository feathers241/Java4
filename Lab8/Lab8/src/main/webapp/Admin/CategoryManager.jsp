...
<body>
  ...
  <main>
    <div class="tabs">
      <div class="tab" data-target="edition">Category Edition</div>
      <div class="tab" data-target="list">Category List</div>
    </div>
    ...
    <form method = "post">
      <div id="edition" class="tab-panel" style = "display:${EditorDisplay}">
        <div class="editor">
          <div>
            <div class="field"><label for="ytid">Category ID</label><input id="ytid" type="text" value = "${Youtubeid}" name = "Youtubeid"></div>
            <div class="field"><label for="title">Category Name</label><input id="title" type="text" value = "${VideoTitle}" name = "VideoTitle"></div>
          
            <div class="actions-row">
              <button class="btn create" id="btnCreate" formaction = "${pageContext.request.contextPath}/CategoryManager/Create">Create</button>
              <button class="btn update" id="btnUpdate" formaction = "${pageContext.request.contextPath}/CategoryManager/Update">Update</button>
              <button class="btn delete" id="btnDelete" formaction = "${pageContext.request.contextPath}/CategoryManager/Delete">Delete</button>
              <button class="btn reset" id="btnReset" formaction = "${pageContext.request.contextPath}/CategoryManager/Reset">Reset</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="list" class="tab-panel"  style = "display:${ListDisplay}" >
        <div style="margin-bottom:8px;display:flex;justify-content:space-between;align-items:center">
          <div style="font-weight:700;color:#c84b22">Category List</div>
          ...
          <tbody>
          	<c:forEach items = "${VideoList}" var = "v">
          		 <tr>
		            ...
		            <td><a href = "${pageContext.request.contextPath}/CategoryManager?videoid=${v.id}">Edit</a></td>
           		 </tr>
          	</c:forEach>
          </tbody>
        </table>
        ...
        <div class="pager">
          <a href = "${pageContext.request.contextPath}/CategoryManager?page=1&display=block"><button class="pgbtn" id="first">|&lt;</button></a>
          <a href = "${pageContext.request.contextPath}/CategoryManager?page=${page > 1 ? page-1 : 1}&display=block"><button class="pgbtn" id="prev">&lt;&lt;</button></a>
          
          <div id="pageInfo" style="padding:6px 10px;color:#666"></div>
          
          <a href = "${pageContext.request.contextPath}/CategoryManager?page=${page < 1 ? 1 : page+1}&display=block"><button class="pgbtn" id="next">&gt;&gt;</button></a>
          <a href = "${pageContext.request.contextPath}/CategoryManager?page=${last}&display=block"><button class="pgbtn" id="last">&gt;|</button></a>
        </div>
      </div>
    </div>
  </main>
